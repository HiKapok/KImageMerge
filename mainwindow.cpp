...
void MainWindow::do_image_merge()
{
    ...
    if(merge.prepare_files(m_sTarghetDir)){
        if(m_bPreferLatter) merge.merge_image(KImageMerge::LatterPrefer, m_sSaveName.toLocal8Bit().data());
        else merge.merge_image(KImageMerge::FormerPrefer, m_sSaveName.toLocal8Bit().data());
...
void MainWindow::set_order(bool sts)
{
    m_bPreferLatter = sts;
}
