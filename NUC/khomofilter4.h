...
{
public:
    KHomoFilter4(double _sharpen = 0.5)
        : m_fGammaHigh(1.)
        , m_fGammaLow(0.)
        , m_fSharpen(_sharpen)
    {
    ...
    bool do_fourier_any(double* _image, uint16_t _width, uint16_t _height)
    {
        ...
        for (uint16_t iY = 0; iY < _width; ++iY)
        {
            ...
            {
                // WARNING: in this implement, make amp_image[][] to be (img^2 + real^2)
                double amplitude = std::pow(frequency_image[iY][iX].imag(), 2.) + std::pow(frequency_image[iY][iX].real(), 2.);
        ...
        double _sum_power = 0.;
        double _max_distance = -(std::numeric_limits<double>::max)();
        for (uint16_t iY = 0; iY < _width; ++iY)
        {
            for (uint16_t iX = 0; iX < _height; ++iX)
            {
                _sum_power += amp_image[iY * _height + iX];
                ...
                if(_max_distance < distance_image[iY * _height + iX]) _max_distance = distance_image[iY * _height + iX];
                if(distance_image[iY * _height + iX] < DBL_EPSILON) distance_image[iY * _height + iX] = DBL_EPSILON;
            }
        }
        _average_distance /= (_width * _height);
        // to reduce the require memory space, we quantify the distance with 0.3
        const double _quantify_ratio = .1;
        uint32_t _max_elements = static_cast<uint32_t>(_max_distance/_quantify_ratio + 1);
        std::vector<std::pair<double, double>> distance_acculate;
        for(uint32_t index = 0;index < _max_elements;++index){
            distance_acculate.push_back(std::make_pair(index * _quantify_ratio, 0.));
        }
        //qDebug()<<_max_elements;
        for (uint16_t iY = 0; iY < _width; ++iY)
        {
            for (uint16_t iX = 0; iX < _height; ++iX)
            {
                distance_acculate[static_cast<uint32_t>(distance_image[iY * _height + iX]/_quantify_ratio)].second += amp_image[iY * _height + iX];
            }
        }
        std::sort(distance_acculate.begin(), distance_acculate.end(), [](std::pair<double, double> _left, std::pair<double, double> _right)->bool{ return _left.second > _right.second; });
        double _sum_accumulate = 0.;
        //for(auto it : distance_acculate){
        //    qDebug()<<it.second;
        //}
        m_fGammaHigh = distance_acculate.front().first;
        for(auto it = distance_acculate.begin(); it != distance_acculate.end(); ++it){

            _sum_accumulate += it->second;
            if(_sum_accumulate > 0.7 * _sum_power){
                m_fGammaLow = it->first;
                break;
            }
        }
//        uint32_t _max_elements = static_cast<uint32_t>(_max_distance/_quantify_ratio + 1);
//        double *distance_acculate = new (std::nothrow) double[_max_elements];
//        if (nullptr == distance_image){
//            delete[] filter_image;
//            delete[] amp_image;
...
//            for (uint16_t iX = 0; iX < _height; ++iX)
//            {
//                distance_acculate[static_cast<uint32_t>(distance_image[iY * _height + iX]/_quantify_ratio)] += amp_image[iY * _height + iX];
//            }
//        }
        //std::sort()
...
//		const double rou_c = 70.;
        qDebug()<<"GammaHigh"<<m_fGammaHigh<<"GammaLow"<<m_fGammaLow<<"Sharpen"<<m_fSharpen;
        for (uint16_t iY = 0; iY < _width; ++iY)
        {
            for (uint16_t iX = 0; iX < _height; ++iX)
            {
                filter_image[iY * _height + iX] = 1. / (1. + std::pow(distance_image[iY * _height + iX] / m_fGammaHigh, 2.)) + 1. / (m_fSharpen * (1. + std::pow(m_fGammaLow/distance_image[iY * _height + iX], 2.)));
            }
        }
