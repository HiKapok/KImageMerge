...
{
public:
    KHomoFilter2(double _gammaHigh = 2.5, double _gammaLow = .1, double _sharpen = 0.42)
...
//		const double rou_c = 70.;
        ...
        for (uint16_t iY = 0; iY < _width; ++iY)
        {
            for (uint16_t iX = 0; iX < _height; ++iX)
            {
                filter_image[iY * _height + iX] = m_fGammaLow + (m_fGammaHigh - m_fGammaLow) * (1. / std::pow((1. + distance_image[iY * _height + iX] / _average_distance), 2.) + 1. / (m_fSharpen * std::pow((1. + _average_distance/distance_image[iY * _height + iX]), 2.)));
            }
        }
    ...
    // H(u,v) = E_l + (E_h - E_l) *( 1. / (1+D(u,v)/D_0)^2 + 1. / (c * (1+D_0/D(u,v))^2) )
    // where D(u,v) is the distance to the filter center and c is the sharpen ratio, and the D_0 is the average od D(u,v)
