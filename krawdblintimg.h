...
{
public:
    typedef uint16_t kimagesize_t;

    KRawDblIntImg(std::string _filename, kimagesize_t _width, kimagesize_t _height, std::string _savefile="D://save_image.DAT")
    ...
    template<typename T>
    static bool read_image_direct(std::string fileName, uint32_t width, uint32_t height,T * image){
        ...
        for (uint32_t index(0); index < width * height; ++index){
            ...
            image[index] = static_cast<T>(tempBuff.get_real_value());
        }
        fs.close();
...
private:
    ...
    kimagesize_t m_iWidth;
    kimagesize_t m_iHeight;
